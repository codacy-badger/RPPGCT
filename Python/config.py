...
''' class cpu_config(config_global):                                                # Configuración del sistema de CPU
    GPIOS           = [(26, True,  True , self.LED  , 'Verde'                   ),  # GPIOS contiene ternas de datos en formato lista:
                       (19, True,  True , self.LED  , 'Amarillo'                ),  # el primer elemento será el número (BCM) de puerto GPIO a manipular,
                       (13, True,  True , self.LED  , 'Naranja'                 ),  # el segundo, el modo (True para salida, False para entrada)
                       ( 6, True,  True , self.LED  , 'Rojo'                    ),  # el tercero, la activación si es de salida (True si es activo a alto nivel o False si es a bajo nivel) o el estado si es de entrada (True si está bajado y False subido)
                       ( 5, True,  True , self.LED  , 'Alarma'                  ),  # y el cuarto, una muy breve descripción de su función
                      ]
...
class dht11_config(config_global):
    GPIOS           = [(25, False, False, self.SONDA, 'Sonda DHT11 de pruebas'  ),
                      ]
...
class domotica_servidor_config(domotica_cliente_config):
    GPIOS           = [(22, False, False, self.BOTON, 'Botón reinicio router'   ),  # En este caso, los puertos GPIO serán dados por pares:
                       ( 4, True,  False, self.RELE , 'Relé reinicio router'    ),  # Las entradas impares orresponderán a los relés que se gestionarán

                       (24, False, False, self.BOTON, 'Botón reinicio switch'   ),  # Las pares, a los pulsadores o equivalentes que irán asociados a dichos relés, para su conmutación
                       (23, True,  False, self.RELE , 'Relé reinicio switch'    ),

                       (17, False, False, self.BOTON, 'Botón reinicio cámara'   ),
                       (27, True,  False, self.RELE , 'Relé reinicio cámara'    ),

                       (13, False, False, self.SONDA, 'Indicador electricidad'  ),
                       (16, True,  False, self.RELE , 'Relé activación router'  ),
                      ]
...
''' class temperatura_config(config_global):
    ...
    GPIOS           = [(16, True,  True , self.LED  , 'Frío'                    ),
                       (20, True,  True , self.LED  , 'Intermedio'              ),
                       (21, True,  True , self.LED  , 'Caliente'                ),
                       (12, True,  True , self.LED  , 'Alarma'                  ),
                      ]
